...
class Visualization(object):
    ...
    def change_xy(pos_now, pos_end, blocklist, routelist, mapsize=50):
        ...
        pos_now = tuple((tmp+pos_now[0], tmp+pos_now[1]))
...
def check_pos(pos_now, pos_end):
    return 0

def transform_vx_vy_w(pos_now, pos_next):
    pass


def vx_vy(receive_route, q):
    """
    ...
    while routelist:
        pos_next = routelist[0]  # 获取目标位置
        pos_now = tuple((0, 0)) # 实际的pos_now，由传感器给我
        # if q.full():
        #     _ = q.get()
        # q.put(pos_now)
        # print("send pos_now: ", pos_now)
        # pos_now格式：(x, y, angle), angle为小车转角
        while True:
            pos_now = tuple((pos_now[0], pos_now[1])) # 实际的pos_now，由传感器给我
            if q.full():
                _ = q.get()
            # print("sending pos_now: ", pos_now)
            q.put(pos_now)
...
def path_planning():
    ...
    processing_control = multiprocessing.Process(target=vx_vy, args=(receive_route, q))
    # 定义一系列必要变量
    blocks = list()  # 包含记忆参数的障碍物列表，一个元素包含(x,y)和rank
    blocklist = list()  # 单纯的(x,y)的障碍物列表
    routelist = list()  # 路径列表
    routelist_new = list()
    # pcd_sub = rospy.Subscriber("/PointClouds",PointCloud2,callback)
    # thread = threading.Thread(target=subpoint)
    flag = 0
    ...
    while ps_now != ps_end:
        ...
        create_block = Blocklist(point_cloud_data, blocks, blocklist, maxrank=2, scale=0.3)  # 类的初始化
        ...
        if not check(blocklist, routelist):  # 开始、重新规划
            ...
            myastar.run()
            routelist_new, routelist = handle(myastar.get_minroute())  # routelist格式：pos_now--->pos_next--->pos_next_next...
