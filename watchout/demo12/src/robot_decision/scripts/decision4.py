...
def give_start_and_end():
    return tuple((0, 0)), tuple((50, 0)),

velpub = rospy.Publisher("cmd_vel",Twist,queue_size=10)

def vx_vy(p_n, p_e, p_next,q=None):
    # p_next = p_n
    # while p_n != p_e:  
    # if not q.empty:
    #     p_next = q.get()
    ...
    # 通讯
    # try:
    #     listener1.waitForTransform("/map","/laser",rospy.Time(0),rospy.Duration(3.0))
    # except (tf.LookupException, tf.ConnectivityException, tf.ExtrapolationException):
    #     continue
    # (trans,rot)=listener1.lookupTransform("/map","/laser",rospy.Time(0))
    
    vel = Twist()
    vel.linear.x = vx 
    vel.linear.y = vy
    vel.angular.z = math.atan(vy/(vx+0.1))*10
    print(math.atan(vy/(vx+0.1))*10)
    velpub.publish(vel)
    odom = rospy.wait_for_message("/odom",Odometry,timeout=rospy.Duration(3.0))
    
    p_n = (math.floor(odom.pose.pose.position.x),math.floor(odom.pose.pose.position.y))
    # p_n = (math.floor(trans[0]),math.floor(trans[1]))
...
def programming(n_bl, n_b, routelist, p_n, p_e, q=None):
    """
    路径规划程序，作为一个线程存在
    :param n_bl: 障碍物列表
    ...
    p_next = -1
    f=1
    while tuple((int(p_n[0]),int(p_n[1]))) != p_e:
        print( tuple((int(p_n[0]),int(p_n[1]))), p_next)
        ...
        # point_cloud_data = pc2.read_points_list(pcd,field_names=('x','y'),skip_nans=True)
        pcl_raw = rospy.wait_for_message("/PointClouds",PointCloud2,timeout=rospy.Duration(3.0))
        point_cloud_data = pc2.read_points_list(pcl_raw,field_names=('x','y'),skip_nans=True)
        ...
        if  f < 2:
            myastar = AStar(MAPSIZE, tuple((int(p_n[0]),int(p_n[1]))), p_e)  # 类的初始化
        ...
        # 通讯，获得当前位置
        # try:
        #     listener.waitForTransform("/map","/laser",rospy.Time(0),rospy.Duration(3.0))
        # except(tf.LookupException, tf.ConnectivityException, tf.ExtrapolationException):
        #     continue
        # (trans,rot) = listener.lookupTransform("/map","/laser",rospy.Time(0))
       
        # odom = rospy.wait_for_message("/odom",Odometry,timeout=rospy.Duration(3.0))
        # p_n = (int(math.floor(odom.pose.pose.position.x)),int(math.floor(odom.pose.pose.position.y)))
        # vx_vy(p_n,p_e,p_next)
...
def main():
    ...
    # q.put(1)
    send, out = multiprocessing.Pipe()

    ps_now, ps_end = give_start_and_end()
    # initpose = rospy.wait_for_message('/odom',Transform,timeout=rospy.Duration(3.0))
    # ps_now = (math.floor(initpose.translation.x),math.floor(initpose.translation.y))
    # p1 = multiprocessing.Process(target=programming,
    #                              args=(now_blocklist, now_blocks, now_routelist, ps_now, ps_end, send))
    # p2 = multiprocessing.Process(target=vx_vy, args=(ps_now, ps_end, out))
    # p1.start()
    # p2.start()
    programming(now_blocklist, now_blocks, now_routelist, ps_now, ps_end)
