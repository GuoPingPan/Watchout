...
class Visualization(object):
    ...
    """
    def __init__(self, blocklist, pos_now, pos_end, mapsize, routelist=None):
...
def main():
    ...
    while True:
        ps_now, ps_end = give_start_and_end()
        pcl_raw = rospy.wait_for_message("/PointClouds",PointCloud2,timeout=rospy.Duration(10))
        point_cloud_data = pc2.read_points_list(pcl_raw,field_names=('x','y'),skip_nans=True)
        create_block = Blocklist(SCALE, point_cloud_data, n_b, n_bl, maxrank=MAXRANK)
        n_bl, n_b = create_block.pcd_discretization(p_n=ps_now, p_e=ps_end)
        vis = Visualization(n_bl, ps_now, ps_end, mapsize=MAPSIZE, routelist=routelist)
        vis.visual()
